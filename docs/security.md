...
この文書は、このローカル開発向け music-effect system host の信頼境界と
最低限のセキュリティ対策をまとめます。

## 前提

- このリポジトリは公開Webサービスではなく、ローカル開発用のsystem hostです。
...
- system app の `?song=<manifest-url>`
...
- song-pack server のCORSは既定で system app のoriginだけを許可します。
...
- system app のポートやoriginを変える場合は、`SONG_PACK_CORS_ORIGINS` を明示してください。
